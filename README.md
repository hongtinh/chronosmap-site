...
**鲸游**（把照片变成活的旅行地图）。视觉与交互沿用 ChronosMap 官网
的设计语言（浅色蓝调、Inter 字重对比、设备截图网格、手机框路线插画、sticky
模糊导航），在其上扩展了工作室首页与鲸游页。
...
| `studio-i18n.js` | 注入工作室首页 / 鲸游 / 品牌名的多语言文案，并把隐私/支持改写为工作室通用版 |
...
工作室首页与鲸游页的完整文案，其余语言的工作室/鲸游文案回退英文（ChronosMap
...
- 鲸游页沿用 ChronosMap 的 `device-shot` / `phone-frame` / `route-line` 组件，
  保持两页视觉一致。
